...
{
  if (argc != 4) {
    std::cerr << "Usage: " << argv[0] << " t(0-square, 1-sphere)" << " m"<<" n"<< std::endl;
    return 1;
  }
  
  int t,m,n;
  std::istringstream(argv[1]) >> t;
  std::istringstream(argv[2]) >> m;
  std::istringstream(argv[3]) >> n;
  Mesh mesh;
  if(t==0)
  // Create a unit square grid divided into m rows and n columns
  mesh = createSquareGrid(1.0f, 1.0f, m, n);
  else
  // Create a unit sphere with m slices and n stacks
  mesh = createSphere(0.6f, m, n);
